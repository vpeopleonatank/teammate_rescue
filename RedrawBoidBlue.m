function RedrawBoidBlue(Boids,BoidsNum,v_Image,v_Alpha,v_ImageE,v_AlphaE,BoidsPlot, imageB_bithuong, alphaB_bithuong, thresholdHP)
...
for BoidIndex = 1 : BoidsNum
    v_angle = 180;
    if (Boids(BoidIndex, 15) > thresholdHP)            
    ...
    elseif(Boids(BoidIndex, 15) <= thresholdHP && Boids(BoidIndex, 15) > 0)
         try
            img_i = imrotate(imageB_bithuong, v_angle );
            alpha_i = imrotate(alphaB_bithuong, v_angle );
        catch Me
            disp(' hi');
            v_angle
        end
